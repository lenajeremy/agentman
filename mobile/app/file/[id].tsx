...
export default function FileScreen() {
  ...
  const [tab, setTab] = useState<"code" | "diff">(() => tabParam === "diff" ? "diff" : "code");
  ...
  return (
    <View style={[styles.page, { paddingTop: insets.top }]}>
      <ContentColumn style={styles.header}>
        <MotionPressable onPress={() => router.back()} style={styles.iconButton} accessibilityRole="button" accessibilityLabel="Back to files">
          <Feather name="chevron-left" size={20} color={color.text} />
        </MotionPressable>
        <FileBadge name={name} directory={false} />
        <View style={styles.heading}>
          <Text style={styles.title} numberOfLines={1}>{name}</Text>
          <Text style={styles.path} numberOfLines={1}>{filePath}</Text>
        </View>
        <MotionPressable onPress={() => setRefresh((value) => value + 1)} style={styles.iconButton} accessibilityRole="button" accessibilityLabel="Refresh file">
      ...
      <ContentColumn style={styles.tabs}>
        ...
        {!result?.image ? (
          ...
          >
            <Feather name="corner-down-left" size={13} color={wrap ? color.onInverse : color.muted} />
            <Text style={[styles.wrapLabel, wrap && styles.wrapLabelOn]}>Wrap</Text>
          </MotionPressable>
        ) : null}
      </ContentColumn>
      {stat ? (
      ...
      {loading ? <ActivityIndicator style={styles.center} color={color.muted} /> : null}
      {error ? <ContentColumn style={styles.message}><Text style={styles.error}>{error}</Text></ContentColumn> : null}
      {!loading && !error && result && (
        <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + space.xl }}>
          {tab === "code" ? (
            result.image && result.mime ? (
              <ImageViewer uri={`data:${result.mime};base64,${result.image}`} />
            ) : (
              <SyntaxCode source={result.text ?? ""} filename={name} wrap={wrap} />
            )
          ) : rows.length > 0 ? (
            <DiffBody rows={rows} wrap={wrap} styles={styles} />
          ) : (
            <ContentColumn style={styles.message}><Text style={styles.empty}>No working tree changes for this file.</Text></ContentColumn>
          )}
          {result.truncated && <ContentColumn style={styles.message}><Text style={styles.note}>Preview truncated. The full file remains on your Mac.</Text></ContentColumn>}
        </ScrollView>
      )}
    </View>
  );
...
function DiffBody({
      ...
      {rows.map((row, index) => {
        ...
        return (
          ...
          >
            ...
            <Text style={styles.diffNo} selectable={false}>{row.oldNumber ?? ""}</Text>
            <Text style={styles.diffNo} selectable={false}>{row.newNumber ?? ""}</Text>
            <Text
              style={[
                styles.diffMark,
            ...
            <Text selectable style={[styles.diffLine, wrap && styles.diffLineWrap]}>
  ...
  return wrap ? (
    ...
    <ScrollView horizontal style={styles.diffFrame} contentContainerStyle={{ minWidth: "100%" }}>
      {body}
    </ScrollView>
  );
}
...
const makeStyles = (c: Palette) => StyleSheet.create({
  ...
  header: { flexDirection: "row", alignItems: "center", gap: space.sm, paddingHorizontal: space.lg, paddingTop: space.sm, paddingBottom: space.md },
  heading: { flex: 1 },
  title: { color: c.text, fontFamily: font.sansBold, fontSize: size.title },
  path: { color: c.muted, fontFamily: font.mono, fontSize: 11 },
  iconButton: { width: 40, height: 40, alignItems: "center", justifyContent: "center", borderRadius: radius.pill, backgroundColor: c.surface, borderWidth: 1, borderColor: c.line },
  tabs: { flexDirection: "row", gap: space.xs, paddingHorizontal: space.lg, paddingBottom: space.md },
  tab: { paddingHorizontal: space.lg, paddingVertical: space.sm, borderRadius: radius.pill, backgroundColor: c.fill },
  ...
  spacer: { flex: 1 },
  wrapToggle: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: space.md, paddingVertical: 7, borderRadius: radius.pill, backgroundColor: c.fill },
  ...
  statBar: { flexDirection: "row", alignItems: "center", gap: space.sm, paddingHorizontal: space.lg, paddingBottom: space.sm },
  statAdd: { color: c.ok, fontFamily: font.monoMedium, fontSize: 12 },
  statRemove: { color: c.errorText, fontFamily: font.monoMedium, fontSize: 12 },
  statNote: { color: c.faint, fontFamily: font.sans, fontSize: 11.5 },

  diffFrame: { backgroundColor: c.surface, borderTopWidth: 1, borderTopColor: c.line },
  diffWrapBody: { width: "100%" },
  diffRow: { flexDirection: "row", alignItems: "flex-start", paddingRight: space.md },
  // Fixed-width gutters: a number column that resizes per row makes the code
  // column jitter as you scroll.
  diffNo: { width: 30, textAlign: "right", paddingRight: 6, fontFamily: font.mono, fontSize: 10.5, lineHeight: 19, color: c.faint },
  diffMark: { width: 14, textAlign: "center", fontFamily: font.monoMedium, fontSize: 12, lineHeight: 19, color: c.faint },
  markAdd: { color: c.ok },
  markRemove: { color: c.errorText },
  diffLine: { fontFamily: font.mono, fontSize: 12, lineHeight: 19, color: c.text },
  diffLineWrap: { flex: 1 },
  addedRow: { backgroundColor: c.okWash },
  removedRow: { backgroundColor: c.errorWash },
  hunkRow: { paddingHorizontal: space.md, paddingVertical: 5, backgroundColor: c.workingWash, borderTopWidth: StyleSheet.hairlineWidth, borderBottomWidth: StyleSheet.hairlineWidth, borderColor: c.workingSoft },
