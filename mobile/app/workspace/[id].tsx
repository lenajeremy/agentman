...
export default function WorkspaceScreen() {
  ...
  const [folder, setFolder] = useState(typeof pathParam === "string" ? pathParam : "");
  ...
  useEffect(() => {
    ...
    setError("");
    setResult(null);
    void store.workspace(sessionId, tab === "files" ? "list_files" : "list_changes", tab === "files" ? folder : "")
  ...
  const openFolder = (name: string) => setFolder(folder ? `${folder}/${name}` : name);
  const openFile = (name: string, initial: "code" | "diff" = "code") => {
    const next = tab === "files" && folder ? `${folder}/${name}` : name;
    router.push(`/file/${encodeURIComponent(sessionId)}?path=${encodeURIComponent(next)}&tab=${initial}`);
  };
  /** Jump straight to an ancestor instead of stepping up one level at a time. */
  const openCrumb = (depth: number) => setFolder(folder.split("/").slice(0, depth).join("/"));
  ...
  const total = tab === "files" ? result?.entries?.length ?? 0 : result?.changes?.length ?? 0;
    ...
    <View style={[styles.page, { paddingTop: insets.top }]}>
      <ContentColumn style={styles.header}>
        <View style={styles.topBar}>
          <MotionPressable onPress={goBack} style={styles.iconButton} accessibilityRole="button" accessibilityLabel={folder ? "Up one folder" : "Back"}>
            <Feather name="chevron-left" size={20} color={color.text} />
          </MotionPressable>
          <View style={styles.heading}>
            <Text style={styles.title} numberOfLines={1} accessibilityRole="header">
              {tab === "changes" ? "Changes" : crumbs.length > 0 ? crumbs[crumbs.length - 1] : "Workspace"}
            </Text>
            <Text style={styles.subtitle} numberOfLines={1}>
              {session ? session.cwd.split("/").at(-1) : sessionId}
              {total > 0 ? ` · ${total}${result?.truncated ? "+" : ""} item${total === 1 ? "" : "s"}` : ""}
            </Text>
          </View>
          <MotionPressable onPress={() => setRefresh((value) => value + 1)} style={styles.iconButton} accessibilityRole="button" accessibilityLabel="Refresh">
        ...
        <View style={styles.tabs}>
          {(["files", "changes"] as const).map((value) => (
            <MotionPressable key={value} onPress={() => setTab(value)} style={[styles.tab, tab === value && styles.tabActive]} accessibilityRole="tab" accessibilityState={{ selected: tab === value }}>
              <Text style={[styles.tabText, tab === value && styles.tabTextActive]}>{value === "files" ? "Files" : "Changes"}</Text>
            </MotionPressable>
          ))}
        </View>
        ...
        {tab === "files" ? (
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.crumbBar} contentContainerStyle={styles.crumbs}>
            <MotionPressable onPress={() => openCrumb(0)} disabled={crumbs.length === 0} accessibilityRole="button" accessibilityLabel="Workspace root">
              <Feather name="home" size={13} color={crumbs.length === 0 ? color.text : color.workingText} />
            </MotionPressable>
            {crumbs.map((crumb, index) => (
              <View key={`${crumb}:${index}`} style={styles.crumbItem}>
                <Feather name="chevron-right" size={12} color={color.faint} />
                <MotionPressable onPress={() => openCrumb(index + 1)} disabled={index === crumbs.length - 1} accessibilityRole="button">
                  <Text style={[styles.crumb, index === crumbs.length - 1 && styles.crumbCurrent]} numberOfLines={1}>{crumb}</Text>
                </MotionPressable>
              </View>
            ))}
          </ScrollView>
        ...
        {showFilter ? (
          <View style={styles.search}>
            ...
            <View style={styles.searchField}>
              ...
              {filter.length === 0 ? (
                <Text style={styles.searchPlaceholder} pointerEvents="none" numberOfLines={1}>
              ...
              <TextInput
                ...
                autoCapitalize="none"
                autoCorrect={false}
                accessibilityLabel={tab === "files" ? "Filter files" : "Filter changes"}
              />
            </View>
            {filter.length > 0 ? (
              <MotionPressable onPress={() => setFilter("")} hitSlop={8} accessibilityRole="button" accessibilityLabel="Clear filter">
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + space.xl }} keyboardShouldPersistTaps="handled">
        <ContentColumn style={styles.content}>
          {loading && <ActivityIndicator style={styles.center} color={color.muted} />}
          {!!error && <Text style={styles.error}>{error}</Text>}

          {!loading && !error && needle.length > 0 && entries.length === 0 && changes.length === 0 && (
            <View style={styles.emptyWrap}><Feather name="search" size={22} color={color.faint} /><Text style={styles.empty}>Nothing matches “{filter.trim()}”.</Text></View>
          )}
          {!loading && !error && !needle && tab === "files" && total === 0 && (
            <View style={styles.emptyWrap}><Feather name="folder" size={22} color={color.faint} /><Text style={styles.empty}>
              {hidden > 0 ? `Nothing to show here. ${hiddenLabel} kept private.` : "This folder is empty."}
            </Text></View>
          )}
          {!loading && !error && !needle && tab === "changes" && total === 0 && (
            <View style={styles.emptyWrap}><Feather name="check-circle" size={22} color={color.faint} /><Text style={styles.empty}>
              {hidden > 0 ? `No changes you can review. ${hiddenLabel} kept private.` : "No working tree changes here."}
            </Text></View>
          )}
          ...
          {tab === "files" && entries.length > 0 ? (
            <View style={styles.list}>
              {entries.map((entry, index) => (
                <MotionPressable
                  key={entry.name}
                  onPress={() => entry.directory ? openFolder(entry.name) : openFile(entry.name)}
                ...
                >
                  ...
                  <Text style={[styles.rowName, entry.directory && styles.rowFolder]} numberOfLines={1}>{entry.name}</Text>
            ...
            <View style={styles.list}>
              {changes.map((change, index) => (
                <MotionPressable
                  ...
                  style={[styles.row, styles.changeRow, index > 0 && styles.rowDivided]}
                  pressedScale={0.995}
                  accessibilityRole="button"
                ...
                >
                  ...
                  <View style={styles.changeIdentity}>
                    <Text style={styles.rowName} numberOfLines={1}>{change.path.split("/").at(-1)}</Text>
                    {change.path.includes("/") ? (
                      <Text style={styles.changeDir} numberOfLines={1}>{change.path.split("/").slice(0, -1).join("/")}</Text>
                    ) : null}
                  </View>
          ...
          {result?.truncated && <Text style={styles.note}>Showing the first entries only.</Text>}
          ...
          {hidden > 0 && total > 0 && (
            <Text style={styles.note}>{hiddenLabel} kept private and not shown.</Text>
          )}
          {!loading && !error && (
...
function ChangeCount({
  ...
  if (change.status.includes("D")) return <Text style={[styles.count, styles.countRemoved]}>deleted</Text>;
  if (change.status.includes("U")) return <Text style={[styles.count, styles.countConflict]}>conflict</Text>;
  if (added === 0 && removed === 0) {
    return <Text style={[styles.count, styles.countNeutral]}>{change.status === "??" ? "new" : "changed"}</Text>;
  }
  return (
    <View style={styles.counts}>
      {added > 0 ? <Text style={[styles.count, removed > 0 ? styles.countEdited : styles.countAdded]}>+{added}</Text> : null}
      {removed > 0 ? <Text style={[styles.count, styles.countRemoved]}>−{removed}</Text> : null}
    </View>
  );
}
...
const makeStyles = (c: Palette) => StyleSheet.create({
  ...
  title: { color: c.text, fontFamily: font.sansBold, fontSize: size.title, letterSpacing: -0.3 },
  subtitle: { color: c.muted, fontFamily: font.mono, fontSize: 11, marginTop: 1 },
  iconButton: { width: 36, height: 36, alignItems: "center", justifyContent: "center", borderRadius: radius.pill, backgroundColor: c.surface, borderWidth: 1, borderColor: c.line },

  tabs: { flexDirection: "row", gap: space.xs, marginTop: space.md },
  tab: { paddingHorizontal: space.lg, paddingVertical: 7, borderRadius: radius.pill, backgroundColor: c.fill },
  tabActive: { backgroundColor: c.inverse },
  tabText: { color: c.muted, fontFamily: font.sansMedium, fontSize: size.caption },
  tabTextActive: { color: c.onInverse },

  crumbBar: { marginTop: space.md, flexGrow: 0 },
  crumbs: { flexDirection: "row", alignItems: "center", gap: 3, paddingRight: space.lg },
  ...
  search: { flexDirection: "row", alignItems: "center", gap: space.sm, marginTop: space.md, height: 40, paddingHorizontal: space.md, borderRadius: radius.md, backgroundColor: c.fill },
  ...
  list: { borderWidth: 1, borderColor: c.line, borderRadius: radius.lg, backgroundColor: c.surface, overflow: "hidden" },
  row: { flexDirection: "row", alignItems: "center", gap: space.md, minHeight: 44, paddingHorizontal: space.md },
  ...
  changeRow: { minHeight: 56, paddingVertical: space.sm, alignItems: "center" },
  ...
  empty: { color: c.muted, fontFamily: font.sans, fontSize: size.caption, textAlign: "center" },
  error: { color: c.errorText, fontFamily: font.sans, fontSize: size.body, paddingVertical: space.md },
  center: { marginTop: space.xl },
  note: { color: c.faint, fontFamily: font.sans, fontSize: 11.5, marginTop: space.md, lineHeight: 16 },
});
