...
Cursor sessions started from the phone use Cursor's ACP interface. They stream
partial replies, keep their history under Agentman's private local state, and
can be resumed after the daemon restarts. Agentman handles ACP permissions,
questions, plan approvals, and interrupts on the phone. These sessions run as
background Cursor processes; `am cursor` remains the way to open Cursor's
interactive terminal UI. Existing terminal chats keep their tmux-based control
and may show a completed reply only when Cursor writes its local chat store.
...
The phone's **+** button can also start a session while the Mac daemon is
online. Choose Claude Code, Codex, Cursor CLI, or OpenCode, browse to a folder
beneath the Mac user's home directory, and edit the first message. The daemon
starts Claude Code and Codex in detached managed tmux sessions. It starts
Cursor through ACP and OpenCode through a local headless API server. The first
message is sent to the selected agent, not generated by Agentman. Hidden
folders, symlinks, and paths outside the home directory are unavailable in
the picker. Cursor IDE chats cannot be created through this path. Codex's
current transcript matching supports one managed Codex session per directory;
the daemon refuses a second rather than risking messages going to the wrong
conversation.
